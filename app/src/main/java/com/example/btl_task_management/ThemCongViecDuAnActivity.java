...
public class ThemCongViecDuAnActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        maNguoiTao = prefs.getInt("MaNguoiDung", -1);
        ...
        if (duAn != null) {
            ...
            if (nguoiTao != null) {
                danhSachThanhVien.add(nguoiTao);
            }
        }
        List<NguoiDung> thanhVienKhac = dbHelper.layThanhVienDuAn(maDuAn);
        for (NguoiDung nd : thanhVienKhac) {
            boolean daTonTai = false;
            for (NguoiDung ndDaCo : danhSachThanhVien) {
                if (ndDaCo.getMaNguoiDung() == nd.getMaNguoiDung()) {
                    daTonTai = true;
                    break;
                }
            }
        ...
        if (tenThanhVien.isEmpty()) {
            tenThanhVien.add("Chưa có thành viên");
        }
        ...
        btnThem.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (tieuDe.isEmpty() || ngayBatDau.isEmpty() || ngayKetThuc.isEmpty()) {
                    Toast.makeText(ThemCongViecDuAnActivity.this, "Vui lòng nhập đầy đủ thông tin", Toast.LENGTH_SHORT).show();
                    return;
                }
                if (danhSachThanhVien.isEmpty()) {
                    Toast.makeText(ThemCongViecDuAnActivity.this, "Dự án chưa có thành viên. Vui lòng thêm thành viên trước", Toast.LENGTH_SHORT).show();
                    return;
                }
                int viTriThanhVien = spinnerThanhVien.getSelectedItemPosition();
                NguoiDung nguoiDuocGiao = danhSachThanhVien.get(viTriThanhVien);
                ...
                if (result > 0) {
                    Toast.makeText(ThemCongViecDuAnActivity.this, "Thêm công việc thành công", Toast.LENGTH_SHORT).show();
                    finish();
                } else {
                    Toast.makeText(ThemCongViecDuAnActivity.this, "Thêm công việc thất bại", Toast.LENGTH_SHORT).show();
                }
            }
        });
